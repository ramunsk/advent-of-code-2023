...
function extrapolateSample(sample: Sample) {
    ...
    for (let i = diffs.length - 2; i >= 0; i--) {
        diffs[i].push(diffs[i].at(-1)! + diffs[i + 1].at(-1)!);
    }
    ...
    sample.push(diffs[0].at(-1)! + sample.at(-1)!);
    // __printArray(sample);
}
...
const total = samples.map((s) => s.at(-1)!).reduce((a, b) => a + b, 0);
console.log('Total is:', total);
// 2098530125 is the right answer
